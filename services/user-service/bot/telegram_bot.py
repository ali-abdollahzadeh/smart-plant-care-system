...
def register_plant_for_user(user_id, name, type_, thresholds, species=None, location=None):
    ...
    if telegram_user_id:
        ...
        async def send_plant_registered():
            try:
                await tg_app.bot.send_message(chat_id=telegram_user_id, text=f"🌱 A new plant '{name}' has been registered for you!")
            except Exception as e:
...
# Initialize user-plant assignments on startup
load_user_plant_assignments()
